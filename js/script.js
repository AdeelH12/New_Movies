let url =
  "https://api.themoviedb.org/3/movie/popular?api_key=f1d314280284e94ff7d1feeed7d44fdf&language=en-US&page=1";
let img = "https://image.tmdb.org/t/p/w500/";

let request = new XMLHttpRequest(); // New instance of XMLHttpRequest
request.open("GET", url, true); // Open the connection using the 'GET' Method
request.onload = function () {
  // Create an onload function this is where the data is displayed on the webpage

  let data = JSON.parse(this.response); // Creating a data variable where our JSON is parsed and stored.
  ...
  if (request.status >= 200 && request.status < 400) {
    ...
    data.results.forEach((results) => {
      ...
      // 78dbdb8ea634f9a57db6a9141b1a0354 Zomato api-key
    });
  }
};
